...
class Solution {
  ...
  static class BinarySearchTree {
    ...
    Node findInOrderSuccessor(Node inputNode) {
      ...
      // we have a right node - call helper function
      if(inputNode.right != null){
        return greatestSuccessor(inputNode.right);
      } else {
      // we dont have right node - traverse up the parent until we find node > than inputNode
        
      if(inputNode.key < root.key) {
         Node temp = inputNode.parent;
         while(temp != null && temp.key<inputNode.key) {
           temp = temp.parent;
         }
         return temp;
      } else {
    ...
    Node greatestSuccessor(Node input) {    
      if(input.left == null){
        return input;
      } else {
        return greatestSuccessor(input.left);
      }
    }
